...
CHANGELOG
  ...
  (Some changes missed here)
  5/2012: DLM modifications to fix bugs in several sections and a slight edit to perturbGromacsTopology to handle setup of calculations which involve both charge and LJ mutations simultaneously.
  5/7/2012: DLM Adding functionality (convertPrmCrdToTopGro) to use acpype as a driver script to convert AMBER coordinate/topology formats to gromacs coordinate/topology formats directly.
  5/11/12: DLM modifying add_ligand_to_topology to handle atom types with asterisks in them.
  6/15/12: DLM modifying parameterizeForAmber to ensure ligand charges are close to zero to high tolerance (OE uses sloppy tolerance) before conversion to prmtop and crd files. 
"""
...
def splitMolFileToFiles( infile, outprefix, outformat='.mol2', SeparateConformers = False, UseTitles = True ):
    ...
    for mol in istream.GetOEMols(): #Loop over molecules (not conformations -- these are looped seperately) in input stream
        if UseTitles:
            ...
            outprefix_t = outprefix+'_%s' % molnum
            title = mol.GetTitle()
            molnum+=1
        ...
        if not SeparateConformers:
            ...
            if ofile in used_filenames: #If we have already used this output file, that means we've got a glitch (see note above) with the multi-conformer handling, and we need to basically append these molecule(s) to it
                ...
                for tmpmol in tempmols:
                    ...
                    #OEWriteMol2File( ostream, tmpmol)
            else:
                ostream.open( ofile )
        ...
        for conformer in mol.GetConfs(): 
            ...
            #OEWriteMol2File( ostream, conformer )
...
def perturbGromacsTopology(topology_filename, reverse=False, molecule = None, perturb_torsions = True, perturb_vdw = True, perturb_charges = True, perturb_atom_indices = None, vdw_decoupling = False, decouple_atom_types = None):
   ...
   for index in indices:
      ...
      if perturb_vdw: 
          ...
          #DLM refinement 5/2/12 -- A state zero only when not also perturbing charges
          atom['chargeB'] = 0.0
          if not perturb_charges:
              atom['charge'] = 0.0
      if perturb_charges: 
...
def add_ligand_to_gro(targetgro, liggro, outgro, resname = 'TMP', add_after_resnum = None):
   ...
   NOTES
    ...
    CHANGELOG
      - 5/1/12 DLM and Pavel Klimovich modified to strip any velocities from gro files 
    """
    ...
    # Strip off velocities from the ligand file.
    liglines = liglines[:2] + [i[:44]+'\n' for i in liglines[2:-1]] + [liglines[-1]] 
    ...
    # Strip off velocities from the target file.                                                                                           
    targetlines = targetlines[:2] + [i[:44]+'\n' for i in targetlines[2:-1]] + [targetlines[-1]] 
    ...
    resnumname='%5s%-4s' % (resnum, resname)
    for line in liglines[2:-1]:
        anum = int( line[15:20].split()[0] )
        newanum = atomnum+anum
        #line = resnumname+line[9:15]+('%5s' % newanum)+line[20:] #DLM 5/1/12 per PVK
        line = resnumname+line[10:15]+('%5s' % newanum)+line[20:]
        newline = '%5s' % (int(line[0:5])+1) + line[5:]
...
def add_ligand_to_topology(prottop,ligtop,complextop):
   ...
   #Don't include forcefield information more than once -- strip it from the ligand if present
   #(Code only works for new-style gromacs forcefields which are always forcefield.itp in some directory)
   present = False
   for line in prottext:
      if 'forcefield.itp' in line:
          present = True
   for (idx, line) in enumerate(ligtext):
      if 'forcefield.itp' in line and present:
          ligtext[idx]='\n' #Set it to be blank - no need to have two includes
             ...
             if name=='atoms':
               ...
               for line in ligsection[name]:
                 ...
                 if m:
                   ...
                   newline="%6s%5s%6s%12s%5s" % (newatnr, m.group('type').strip(), newresnr, m.group('middle').strip(), newcgnr) + m.group('end')+'\n'
                   #Append this line
               ...
               for line in ligsection[name]:
                 ...
                 if m:
                   ...
                   newline="%6s%7s" % (newatomi, newatomj) + m.group('end')+'\n'
                   newtop.append(newline)         
                ...
                for line in ligsection[name]:
                  ...
                  if m:
                    ...
                    newline="%6s%7s%7s" % (newatomi, newatomj, newatomk) + m.group('end')+'\n'
                  ...
                  if ligsection.has_key(name):
                    for line in ligsection[name]:
                      ...
                      if m:
                        ...
                        newline="%6s%7s%7s%7s" % (newatomi, newatomj, newatomk, newatoml) + m.group('end')+'\n'                    
                        newtop.append(newline)
...
def convertPrmCrdToTopGro( prmtop, crd, topology, gro, style = 'New', sorted = True, verbose = False ):
    """Use acpype to convert an AMBER parameter (.prmtop) and coordinate (.crd) file to GROMACS .top and .gro files.

ARGUMENTS:
- prmtop: Input AMBER parameter file
- crd: Input AMBER coordinate file
- topology: Name of output GROMACS topology file to write
- gro: Name of output GROMACS .gro file to write

OPTIONAL:
- style: Default 'New'. String ('New'/'Old') for version 4.5 format (New) or older formats (Old) -- old are compatible with new, but not vise versa; under 4.5 or later dihedral parameters are specified more simply in GROMACS
- sorted: (Boolean). Default True -- sort GROMACS files by charge group? 

REQUIREMENTS:
- acpype.py must be in MMTOOLSPATH
...
"""
    ...
    acpypi =  os.path.join(os.getenv('MMTOOLSPATH'), 'converters', 'acpype.py')
    command = '%(acpypi)s -p temp.prmtop -x temp.crd -b OUT' % vars()
    if sorted==True:
       command+= ' --sorted'
    if style == 'New':
       command += ' --gmx45'
    ...
    shutil.copy(prmtop, os.path.join(dir, 'temp.prmtop') )
    shutil.copy( crd, os.path.join(dir, 'temp.crd')  )
 
    #Make sure input and output files use absolute path
    abstop = os.path.abspath(topology)
    absgro = os.path.abspath( gro)

    #Change dir to temp dir
    os.chdir(dir)

    if verbose: 
         print "Running in temporary directory %s..." % dir
         print "Command:\n",
         print command                                                                                                               
    acpypi_output = commands.getoutput(command)                                                                                             
    if verbose: print acpypi_output                                                                                        

    #Copy files back and remove temporary directory
    try:
      shutil.copy( 'OUT_GMX.gro', absgro )
      shutil.copy( 'OUT_GMX.top', abstop )
      shutil.rmtree( dir ) #Clean up if successful
    except IOError:
      print "Error finding output top or gro file in temporary directory %s after running command '%s'; aborting." % ( dir, command )
      print "Acpype output was: ", acpypi_output
      os.chdir(startdir)
...
def round_charges( mol2file, verbose = True ):
    ...
    totchg = relativefeptools.totalPartialCharge( oemol )
    totintchg = relativefeptools.totalIntegralPartialCharge( oemol )
